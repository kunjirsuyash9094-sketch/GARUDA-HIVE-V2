...
//
// NOTE: payload structs below are in MAVLink *wire order* (fields sorted by
// type size descending, extensions last), NOT the XML declaration order.
// ---------------------------------------------------------------------------
...
static constexpr uint8_t SIM_SYS_ID  = 1;   // match autopilot sysid for HIL
static constexpr uint8_t SIM_COMP_ID = 51;  // simulator component id
...
// CRC extra lookup for messages we expect to receive/validate
[[nodiscard]] inline std::optional<uint8_t> crc_extra_for(uint32_t msg_id) noexcept {
    switch (msg_id) {
        case MSG_ID_HEARTBEAT:             return CRC_EXTRA_HEARTBEAT;
        case MSG_ID_HIL_SENSOR:            return CRC_EXTRA_HIL_SENSOR;
        case MSG_ID_HIL_GPS:               return CRC_EXTRA_HIL_GPS;
        case MSG_ID_HIL_STATE_QUATERNION:  return CRC_EXTRA_HIL_STATE_QUATERNION;
        case MSG_ID_HIL_ACTUATOR_CONTROLS: return CRC_EXTRA_HIL_ACTUATOR_CONTROLS;
        case MSG_ID_SERVO_OUTPUT_RAW:      return CRC_EXTRA_SERVO_OUTPUT_RAW;
        case MSG_ID_HIL_RC_INPUTS_RAW:     return CRC_EXTRA_HIL_RC_INPUTS_RAW;
...
// ---------------------------------------------------------------------------
// Copy a (possibly v2-truncated) payload into a typed struct.
// MAVLink v2 strips trailing zero bytes on the wire; missing bytes are
// semantically zero, so we zero-init and copy what we received.
// ---------------------------------------------------------------------------
template<typename T>
[[nodiscard]] inline T unpack_payload(const Frame& f) noexcept {
    T out{};
    std::memcpy(&out, f.payload.data(),
                std::min<size_t>(f.payload_len, sizeof(T)));
    return out;
}

// ---------------------------------------------------------------------------
// Framer — builds wire bytes from a payload + message ID.
// MAVLink v2 allows truncating trailing zero payload bytes; we keep the
// full length for simplicity (always valid).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Parser — stateful byte-by-byte MAVLink v2 parser with CRC validation.
// Frames whose msg_id is unknown to crc_extra_for() are dropped (we cannot
// validate them and the bridges do not use them).
// ---------------------------------------------------------------------------
class Parser {
public:
    ...
    // Feed one byte; returns a complete validated Frame if one is ready
        ...
        switch (_state) {
            ...
            case State::CRC1: {
                ...
                const uint16_t crc_recv =
                    static_cast<uint16_t>(_crc_lo) |
                    (static_cast<uint16_t>(b) << 8);
                const auto extra = crc_extra_for(_frame.msg_id);
                if (!extra) break; // unknown message — drop
                CRC final = _crc;
                final.accumulate(*extra);
                if (final.value != crc_recv) break; // corrupt — drop
                Frame f = _frame;
                _frame = {};
                return f;
            }
...
// ---------------------------------------------------------------------------
// Payload structs — MAVLink v2 *wire order*, little-endian, packed
// ---------------------------------------------------------------------------
#pragma pack(push, 1)

// msg 107 — all base fields 8/4-byte so wire order == XML order
struct HilSensorPayload {
    uint64_t time_usec{};
    float xacc{}, yacc{}, zacc{};           // m/s² (FRD body)
    float xgyro{}, ygyro{}, zgyro{};        // rad/s (FRD body)
    float xmag{}, ymag{}, zmag{};           // Gauss (FRD body)
    float abs_pressure{};                   // hPa
    float diff_pressure{};                  // hPa
    float pressure_alt{};                   // m
    float temperature{};                    // °C
    uint32_t fields_updated{0x1FFF};        // all fields
    uint8_t  id{0};                         // extension
};

// msg 113 — wire order sorts fix_type/satellites to the end
struct HilGpsPayload {
    ...
    uint8_t  id{0};           // extension
    uint16_t yaw{};           // extension, cdeg (0 = not available)
};
...
// msg 93 — wire order puts the two uint64s first
...
// Wire-format sizes from the MAVLink common dialect — do not change.
static_assert(sizeof(HilSensorPayload)           == 65, "HIL_SENSOR wire size");
static_assert(sizeof(HilGpsPayload)              == 39, "HIL_GPS wire size");
static_assert(sizeof(HilStateQuaternionPayload)  == 64, "HIL_STATE_QUATERNION wire size");
static_assert(sizeof(HilActuatorControlsPayload) == 81, "HIL_ACTUATOR_CONTROLS wire size");
static_assert(sizeof(ServoOutputRawPayload)      == 37, "SERVO_OUTPUT_RAW wire size");
static_assert(sizeof(HeartbeatPayload)           == 9,  "HEARTBEAT wire size");
