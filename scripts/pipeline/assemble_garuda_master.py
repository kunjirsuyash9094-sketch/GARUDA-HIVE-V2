...
def assemble_master_asset():
    ...
    # -------------------------------------------------------------------------
    # 7. Avionics & Strobe Lighting (6x Antennas + GNSS Dome + Red Beacon)
    # -------------------------------------------------------------------------
    av_parts = []
    for ax, az, ah in [(-0.06, 0.14, 0.10), (0.06, 0.14, 0.10), (-0.03, 0.18, 0.12), (0.03, 0.18, 0.12), (-0.08, 0.21, 0.09), (0.08, 0.21, 0.09)]:
        av_parts.append(create_cylinder_mesh(radius=0.006, height=0.014, segments=12, center_x=ax, center_y=0.078, center_z=az, axis='y'))
        av_parts.append(create_oriented_cylinder([ax, 0.085, az], [ax, 0.085 + ah, az], 0.0022, 12))

    av_parts.append(create_cylinder_mesh(radius=0.024, height=0.012, segments=24, center_x=0.0, center_y=0.086, center_z=0.110, axis='y'))
    av_parts.append(create_sphere_mesh(radius=0.022, rings=12, sectors=20, center_x=0.0, center_y=0.090, center_z=0.110))
    ...
    p_bb, n_bb, i_bb = combine_meshes([create_cylinder_mesh(radius=0.016, height=0.014, segments=20, center_x=0.0, center_y=0.098, center_z=0.030, axis='y')])
    ...
    p_bd, n_bd, i_bd = combine_meshes([create_sphere_mesh(radius=0.012, rings=12, sectors=16, center_x=0.0, center_y=0.110, center_z=0.030)])
