...
func _refresh() -> void:
    ...
    var flying: String     = telem.get("sitl_source", "none")

    var text = "[font_size=13]"

    # ---- Firmware status row -----------------------------------------------
    var flying_tag = ("[color=green]%s FLYING[/color]" % flying.to_upper()) \
        if telem.get("sitl_active", false) else "[color=gray]internal FC[/color]"
    text += "[b]SITL[/b]  select=[b]%s[/b]  %s\n" % [active.to_upper(), flying_tag]
