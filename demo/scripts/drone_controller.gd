...
## Physics tick rate. Firmware SITL needs a fast sensor stream:
## ArduCopter wants ~400 Hz, PX4's EKF wants 250 Hz+. 60 is fine for the
## internal PID only.
@export_range(60, 1000, 10) var physics_hz: int = 400

func _ready() -> void:
	if drone == null and not drone_path.is_empty():
		drone = get_node_or_null(drone_path)
	if hud_label == null and not hud_path.is_empty():
		hud_label = get_node_or_null(hud_path)
