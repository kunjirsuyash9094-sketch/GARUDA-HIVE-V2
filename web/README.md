# Garuda Hive in the browser (Phase 4)
...
`web/core/drone_core.{hpp,cpp}` is the Garuda Hive physics **decoupled from Godot**:
...
exposes a small C API; Emscripten compiles it to `garuda_core.js` + `.wasm`.
...
./web/build_wasm.sh          # -> web/public/garuda_core.js + .wasm
```

## Run locally

```bash
cp web/public/garuda_core.* web/     # put the wasm next to index.html
...
This is an independent integrator sharing Garuda Hive's force models. It is
